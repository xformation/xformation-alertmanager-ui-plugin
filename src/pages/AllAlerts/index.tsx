...
export class AllAlerts extends React.Component<any, any> {
                  ...
                  <UncontrolledPopover trigger="legacy" placement="bottom" target={`PopoverFocus-${alert.guid}`}>
                    <PopoverBody>
                      <Rbac parentName={config.PARENT_NAME} childName="allalerts-index-alerttbl-createticketbtn">
                        <Link
                          className=" "
                          to={`${PLUGIN_BASE_URL}/all-tickets?guid=` + alert.guid + '&alertName=' + alert.name}
                        >
                          Create Ticket
                        </Link>
                      </Rbac>
  ...
  fetchData = () => {
    RestService.getData(config.GET_ALL_ALERT_FROM_ELASTIC, null, null).then((response: any) => {
      // let ary = [];
  ...
  createSelectbox = (data: any) => {
    ...
    if (data) {
      ...
      for (let i = 0; i < length; i++) {
        retData.push(<option value={data[i].value}>{data[i].label}</option>);
      }
    }
    return retData;
  };
      ...
      <div className="all-alerts-container">
        ...
        <div className="alert-page-container">
          <div className="common-container">
            <Link to={`${PLUGIN_BASE_URL}/manage-alert-rule`} className="alert-white-button">
              <i className="fa fa-plus"></i>&nbsp;&nbsp; New Alert Rule
            </Link>
            <a className="alert-white-button" onClick={this.refreshData}>
              <i className="fa fa-refresh"></i>&nbsp;&nbsp; Refresh
            </a>
            <Link to={`${PLUGIN_BASE_URL}/manage-alert-rule`} className="alert-white-button float-right">
